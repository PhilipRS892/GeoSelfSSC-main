...
class BTSWrapper(nn.Module):
    def __init__(self, renderer, config, eval_nvs=False) -> None:
        ...
        torch.inverse(torch.ones((1, 1), device="cuda:1"))
    ...
    def forward(self, data):
        ...
        segs = torch.stack(data["segs_gt_map"], dim=1)                          # n, n_segs, h, w
        depths = torch.stack(data["depth_gt_map"], dim=1).float()               # n, v, h, w
        depths_std = torch.stack(data["depth_std_gt_map"], dim=1).float()    # n, v, h, w
        normals = torch.stack(data["normal_gt_map"], dim=1).float()          # n, v, h, w, 3
        ...
        with profiler.record_function("trainer_encode-grid"):
            self.renderer.net.compute_grid_transforms(projs[:, ids_encoder], poses[:, ids_encoder])  # nothing
        ...
        with profiler.record_function("trainer_sample-rays"):
            # all_rays:(B, ray_bs, 8)
            # all_rgb_gt:(B, ray_bs, 3)
            all_rays, all_rgb_gt = sampler.sample(images_ip[:, ids_loss], poses[:, ids_loss], projs[:, ids_loss],
                                                  segs=segs[:, ids_loss], depths=depths[:, ids_loss], normals=normals[:, ids_loss])

        with profiler.record_function("trainer_sample_segmentation-rays"):
            segmentation_rays, seg_rgb_gt, segmentation_gt, depth_gt, depth_std_gt, normal_gt, patch_to_image = sampler.sample(
                images_ip[:,:n_segs], poses[:, :n_segs], projs[:, :n_segs], segs[:, :n_segs], depths[:, :n_segs], depths_std[:, :n_segs], normals[:,:n_segs],
        ...
        if self.prediction_mode == "multiscale":
            ...
            with profiler.record_function("trainer_segmentation_render"):
                render_segmentation_dict = self.renderer(segmentation_rays, depth_gt=depth_gt, normal_gt=normal_gt, depth_std_gt=depth_std_gt,
                                                         want_weights=True, want_alphas=True,
                                                          want_rgb_samps=True, predict_segmentation=True, predict_normal=True,
                                                         sample_from_uniform=None, sample_from_depth=True)
            with profiler.record_function("trainer_render"):
                render_dict = self.renderer(all_rays, want_weights=True, want_alphas=True, want_rgb_samps=True, sample_from_uniform=True)
            ...
            render_segmentation_dict["depth_gt"] = depth_gt
            render_segmentation_dict["normal_gt"] = normal_gt
            ...
            with profiler.record_function("trainer_reconstruct_segmentation"):
                render_segmentation_dict = sampler.reconstruct(render_segmentation_dict, reconstruct_segmentation=True, reconstruct_normal=True)
            ...
            data["rgb_gt"] = render_dict["rgb_gt"]
            data["rays"] = render_dict["rays"]
            data["segmentation"].append(render_segmentation_dict["coarse"])  # segs deps nors 的渲染结果都在这里
            ...
            data["depth_gt"] = render_segmentation_dict["depth_gt"]
            data["normal_gt"] = render_segmentation_dict["normal_gt"]
        ...
        # 遍历所有键值对
        cpu_keys = [k for k, v in data.items() if isinstance(v, torch.Tensor) and v.device == torch.device("cpu")]
        for k in cpu_keys:
            data[k] = data[k].to(device)
            # data[k] = data[k].cuda()
            # data[k] = to(data[k], idist.device)
