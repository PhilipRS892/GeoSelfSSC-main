...
class PatchRaySampler(RaySampler):
    ...
    def sample(self, images, poses, projs, segs=None, depths=None, depths_std=None, normals=None, sample_segs=False, sample_depths=False, sample_normals=False):
        n, v, c, h, w = images.shape
        ...
        for n_ in range(n):
            ...
            sample_segs_gt = []
            sample_depths_gt = []
            sample_depths_std_gt = []
            sample_normals_gt = []

            for v_, y, x in zip(patch_coords_v, patch_coords_y, patch_coords_x):
                # 从4个视角里面交叉取样？  是的，因为要从N_render里面采样rgb去渲染N_loss
                # 一次性取样8*8个点 原图的rgb做gt，同时做成rays
                ...
                if sample_depths:
                    depth_gt_patch = depths[n_][v_, y:y + self.patch_size_y, x:x + self.patch_size_x].reshape(-1)
                    sample_depths_gt.append(depth_gt_patch)
                    depths_std_gt_patch = depths_std[n_][v_, y:y + self.patch_size_y, x:x + self.patch_size_x].reshape(-1)
                    sample_depths_std_gt.append(depths_std_gt_patch)

                if sample_normals:
                    normals_gt_patch = normals[n_][v_, y:y + self.patch_size_y, x:x + self.patch_size_x, :].reshape(-1, self.channels)
                    sample_normals_gt.append(normals_gt_patch)
            ...
            if sample_depths:
                sample_depths_gt = torch.cat(sample_depths_gt, dim=0)
                sample_depths_std_gt = torch.cat(sample_depths_std_gt, dim=0)

            if sample_normals:
                sample_normals_gt = torch.cat(sample_normals_gt, dim=0)


            all_rgb_gt.append(sample_rgb_gt)
            all_rays.append(sample_rays)
            all_segs_gt.append(sample_segs_gt)
            all_depths_gt.append(sample_depths_gt)
            all_depths_std_gt.append(sample_depths_std_gt)
            all_normals_gt.append(sample_normals_gt)
        ...
        if sample_depths:
            all_depths_gt = torch.stack(all_depths_gt)
            all_depths_std_gt = torch.stack(all_depths_std_gt)

        if sample_normals:
            all_normals_gt = torch.stack(all_normals_gt)

        if sample_segs:
            return all_rays, all_rgb_gt, all_segs_gt, all_depths_gt, all_depths_std_gt, all_normals_gt, patch_coords_v
        else:
            return all_rays, all_rgb_gt

    def reconstruct(self, render_dict, channels=None, reconstruct_segmentation=False, reconstruct_normal=False):
        coarse = render_dict["coarse"]
        fine = render_dict["fine"]
        ...
        if reconstruct_segmentation:
            ...
            f_segmentation = fine["segs"]
            segmentation_gt = render_dict["segmentation_gt"]
            # c_segmentation_raw = coarse["segs_raw"]
            n_classes = c_segmentation.shape[-1]
            # n_samples = c_segmentation_raw.shape[-2]

        if reconstruct_normal:
            c_normal = coarse["nors"]
            f_normal = fine["nors"]
            depth_gt = render_dict["depth_gt"]
            normal_gt = render_dict['normal_gt']
        ...
        if reconstruct_segmentation:
            coarse["segs"] = c_segmentation.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, n_classes)
            fine["segs"] = f_segmentation.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, n_classes)
            render_dict["segmentation_gt"] = segmentation_gt.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, 1)

        if reconstruct_normal:
            coarse["nors"] = c_normal.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, 3)
            fine["nors"] = f_normal.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, 3)
            render_dict['depth_gt'] = depth_gt.view(n, self._patch_count, self.patch_size_y, self.patch_size_x)
            render_dict["normal_gt"] = normal_gt.view(n, self._patch_count, self.patch_size_y, self.patch_size_x, 3)
...
class ImageRaySampler(RaySampler):
    ...
    def reconstruct(self, render_dict, channels=None, reconstruct_segmentation=False, reconstruct_normal=False):
        ...
        if reconstruct_normal:
            c_normal = coarse["nors"]  # n, n_pts, 3
        ...
        if reconstruct_normal:
            coarse["nors"] = c_normal.view(n, v_in, self.height, self.width, v_render, 3)
