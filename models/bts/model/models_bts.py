...
from models.common.model.mlp_util import make_mlp, make_intercept_model, make_segnet, make_nornet
...
class BTSNet(torch.nn.Module):
    def __init__(self, conf):
        ...
        if self.segmentation_mode == 'KITTI-360':
            ...
            self.mlp_normal = make_nornet(d_in, d_out=3, d_hidden_list=[64])
    ...
    def encode(self, images, Ks, poses_c2w, ids_encoder=None, ids_render=None, images_alt=None, combine_ids=None):
        ...
        self.grid_c_imgs = images_render  # color是来自于N_render
    ...
    def forward(self, xyz, coarse=True, viewdirs=None, far=False, only_density=False, predict_segmentation=False, predict_normal=False):
        """
        ...
        :param predict_normal, if true also return the normal distribution for all the points
        ...
        with profiler.record_function("model_inference"):
            ...
            if predict_normal:
                nors = self.mlp_normal(mlp_input)
                nors = F.softplus(nors)
            ...
            if not only_density:
                _, _, _, c = rgb.shape
                rgb = rgb.permute(0, 2, 1, 3).reshape(n, n_pts, nv * c)         # (n, pts, nv * 3)  实际上是12 因为从4张N_render那里采样了颜色，所以3*4
        ...
        if predict_segmentation and predict_normal:
            return rgb, invalid, sigma, segs, nors
